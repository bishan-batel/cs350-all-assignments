...
DebugShape& LineSegment::DebugDraw() const { return gDebugDrawer->DrawLine(*this); }
...
DebugShape& Ray::DebugDraw(float t) const { return gDebugDrawer->DrawRay(*this, t); }
...
Matrix3 ComputeCovarianceMatrix(const std::vector<Vector3>& points) {
  ...
  const Vector3 mean{
    std::accumulate(points.begin(), points.end(), Vector3{}, std::plus<Vector3>{}) / static_cast<float>(points.size()),
  };
...
bool Sphere::ContainsPoint(const Vector3& point) { return PointSphere(point, mCenter, mRadius); }
...
DebugShape& Sphere::DebugDraw() const { return gDebugDrawer->DrawSphere(*this); }
...
DebugShape& Triangle::DebugDraw() const { return gDebugDrawer->DrawTriangle(*this); }

//-----------------------------------------------------------------------------Plane
Plane::Plane() { mData = Vector4::cZero; }

Plane::Plane(const Vector3& p0, const Vector3& p1, const Vector3& p2) { Set(p0, p1, p2); }
...
void Plane::Set(const Vector3& normal, const Vector3& point) {
  ...
  if (n_normal.Length() != 1.f) { n_normal.Normalize(); }
...
DebugShape& Plane::DebugDraw(float sizeX, float sizeY) const { return gDebugDrawer->DrawPlane(*this, sizeX, sizeY); }
...
DebugShape& Frustum::DebugDraw() const { return gDebugDrawer->DrawFrustum(*this); }
